...
constexpr char FILE_FORMAT[] = "ChainOSCmini-system-settings";
...
void logResult(const char* operation, size_t fileBytes, const char* result,
  ...
  Serial.printf(
      "[ChainOSCmini][SYSTEM] operation=%s path=%s file_bytes=%u "
...
bool systemSettingsSetup() {
  ...
  if (!deviceFileStorageBegin()) {
    Serial.println(
        "[ChainOSCmini][SYSTEM] operation=setup result=failed reason=mount_failed");
    return false;
  }
  ...
  initialized = true;
  Serial.println(
      "[ChainOSCmini][SYSTEM] migration source=nvs target=littlefs result=ok");
  return true;
}
