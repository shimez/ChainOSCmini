...
String fileStem(const String& identity) {
  if (identity == "dualkey:1") return "Key1";
  if (identity == "dualkey:2") return "Key2";
...
void logResult(const char* operation, const char* type, const String& identity,
  ...
  Serial.printf(
      "[ChainOSCmini][LITTLEFS] operation=%s type=%s identity=%s path=%s "
...
bool deviceFileStorageBegin() {
  ...
  if (!mounted) {
    Serial.println("[ChainOSCmini][LITTLEFS] mount result=failed");
    return false;
  }
...
void deviceFileStorageLogUsage(const char* phase) {
  ...
  Serial.printf("[ChainOSCmini][LITTLEFS] phase=%s total_bytes=%u used_bytes=%u free_bytes=%u\n",
...
size_t deviceFileStorageList(const char* type, String* identities,
  ...
  directory.close();
  Serial.printf("[ChainOSCmini][LITTLEFS] operation=list type=%s files=%u\n",
