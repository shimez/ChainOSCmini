...
bool load(const String& identity, JoystickSetting& setting, bool& found) {
  found=false; Preferences p; const String ns=deviceNamespace(identity); if(!p.begin(ns.c_str(),true)) return false;
  if(p.getString("ver","")!=STORAGE_VERSION || p.getString("id","")!=identity){p.end();return false;} found=true; JoystickSetting c=setting;
  c.displayName=p.getString("name",""); c.xAddress=p.getString("xaddr",""); c.yAddress=p.getString("yaddr",""); c.deadband=p.getInt("dead",3);
  c.invertX=p.getBool("invx",false); c.invertY=p.getBool("invy",false);
  c.outputMin=p.getFloat("omin",0); c.outputMax=p.getFloat("omax",1); c.outputType=static_cast<ValueType>(constrain(p.getUChar("otype",TYPE_FLOAT),(int)TYPE_FLOAT,(int)TYPE_STRING));
  c.clickMode=p.getUChar("cmode",MODE_PRESS_RELEASE)==MODE_SEQUENCE?MODE_SEQUENCE:MODE_PRESS_RELEASE; c.pressMessageCount=p.getUChar("pc",0); c.releaseMessageCount=p.getUChar("rc",0);
  if(c.pressMessageCount+c.releaseMessageCount>MAX_KEY_OSC_MESSAGES){p.end();return false;}
  for(uint8_t i=0;i<c.pressMessageCount;++i){c.pressMessages[i].address=p.getString(indexedKey("pa",i).c_str(),"");c.pressMessages[i].valueStr=p.getString(indexedKey("pv",i).c_str(),"");c.pressMessages[i].valueType=static_cast<ValueType>(constrain(p.getUChar(indexedKey("pt",i).c_str(),TYPE_FLOAT),(int)TYPE_FLOAT,(int)TYPE_STRING));}
  for(uint8_t i=0;i<c.releaseMessageCount;++i){c.releaseMessages[i].address=p.getString(indexedKey("ra",i).c_str(),"");c.releaseMessages[i].valueStr=p.getString(indexedKey("rv",i).c_str(),"");c.releaseMessages[i].valueType=static_cast<ValueType>(constrain(p.getUChar(indexedKey("rt",i).c_str(),TYPE_FLOAT),(int)TYPE_FLOAT,(int)TYPE_STRING));}
  c.clickSequence.address=p.getString("sa","");c.clickSequence.valueType=static_cast<ValueType>(constrain(p.getUChar("stype",TYPE_FLOAT),(int)TYPE_FLOAT,(int)TYPE_STRING));c.clickSequence.start=p.getFloat("start",0);c.clickSequence.end=p.getFloat("end",10);c.clickSequence.step=p.getFloat("step",1);p.end();
  if(!valid(c)) return false; keySettingsNormalizeSequence(c.clickSequence); setting=c; return true;
}
bool write(const JoystickSetting& s){Preferences p;const String ns=deviceNamespace(s.identity);if(!p.begin(ns.c_str(),false))return false;p.clear();p.putString("ver",STORAGE_VERSION);p.putString("id",s.identity);p.putString("name",s.displayName);p.putString("xaddr",s.xAddress);p.putString("yaddr",s.yAddress);p.putInt("dead",s.deadband);p.putBool("invx",s.invertX);p.putBool("invy",s.invertY);p.putFloat("omin",s.outputMin);p.putFloat("omax",s.outputMax);p.putUChar("otype",s.outputType);p.putUChar("cmode",s.clickMode);p.putUChar("pc",s.pressMessageCount);p.putUChar("rc",s.releaseMessageCount);for(uint8_t i=0;i<s.pressMessageCount;++i){p.putString(indexedKey("pa",i).c_str(),s.pressMessages[i].address);p.putString(indexedKey("pv",i).c_str(),s.pressMessages[i].valueStr);p.putUChar(indexedKey("pt",i).c_str(),s.pressMessages[i].valueType);}for(uint8_t i=0;i<s.releaseMessageCount;++i){p.putString(indexedKey("ra",i).c_str(),s.releaseMessages[i].address);p.putString(indexedKey("rv",i).c_str(),s.releaseMessages[i].valueStr);p.putUChar(indexedKey("rt",i).c_str(),s.releaseMessages[i].valueType);}p.putString("sa",s.clickSequence.address);p.putUChar("stype",s.clickSequence.valueType);p.putFloat("start",s.clickSequence.start);p.putFloat("end",s.clickSequence.end);p.putFloat("step",s.clickSequence.step);p.end();JoystickSetting v=s;bool found=false;return load(s.identity,v,found)&&found&&same(s,v);}
...
bool joystickSettingsDelete(const String& identity){size_t found=settingCount;for(size_t i=0;i<settingCount;++i)if(settings[i].identity==identity){if(settings[i].connectedPortMask)return false;found=i;break;}if(found==settingCount)return false;Preferences p;String ns=deviceNamespace(identity);if(p.begin(ns.c_str(),false)){p.clear();p.end();}for(size_t i=found+1;i<settingCount;++i)settings[i-1]=settings[i];--settingCount;settings[settingCount]=JoystickSetting();saveKnown();return true;}
