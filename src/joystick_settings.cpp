...
bool loadLegacy(const String& identity, JoystickSetting& setting, bool& found) {
...
bool load(const String& identity, JoystickSetting& setting, bool& found){const DeviceFileLoadResult result=deviceFileStorageLoad(setting);if(result==DeviceFileLoadResult::Loaded){found=true;return true;}if(result==DeviceFileLoadResult::Error){found=true;return false;}if(!loadLegacy(identity,setting,found)||!found)return false;if(deviceFileStorageSave(setting))Serial.printf("[ChainOSCmini][JOYCFG] migrated identity=%s source=NVS target=LittleFS\n",identity.c_str());return true;}
