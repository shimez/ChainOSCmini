...
function focusEncoderAfterMigration(identity){sessionStorage.setItem('chainoscmini-focus-encoder-migration',identity)}
...
String pageStart(const char* title) {
  ...
  html += F("<link rel='icon' type='image/svg+xml' href='data:image/svg+xml;base64,'>");
  html += F("<title>");
  html += title;
  html += F("</title><style>");
...
String provisioningPageStart() {
  ...
  html += F("'><head><meta charset='utf-8'>"
            ...
            "<title>ChainOSCmini Wi-Fi Setup</title><style>"
            "body{font-family:sans-serif;margin:16px;background:#f5f5f5;color:#18212f}"
...
void appendKeyCard(String& html, const KeySetting& setting, size_t cardIndex,
  ...
  else if (setting.builtIn) {
    html += String("<span class='badge badge-type'>#") + (cardIndex + 1) +
            " DualKey</span>";
...
void appendEncoderCard(String& html, const EncoderSetting& persistedSetting,
  ...
  html += "<div class='device-head'><h2><button id='collapse-" + idx +
          "' class='collapse-button' type='button' aria-expanded='true' onclick=\"toggleDevice('" + idx + "','" + htmlEscape(collapseKey) + "')\">&#9660;</button>" + chainPositionBadge(portNumber, portIndex, "Encoder") + " " + htmlEscape(setting.displayName) + "</h2>";
...
void appendAngleCard(String& html, const AngleSetting& setting,
  ...
  html += "<div class='card device' data-device-index='" + idx +
          "' data-collapse-key='" + htmlEscape(collapseKey) + "'>";
  html += "<div class='device-head'><h2><button id='collapse-" + idx +
          "' class='collapse-button' type='button' aria-expanded='true' onclick=\"toggleDevice('" + idx + "','" + htmlEscape(collapseKey) + "')\">&#9660;</button>" + chainPositionBadge(portNumber, portIndex, "Angle") + " " + htmlEscape(setting.displayName) + "</h2>";
  html += "<div class='device-menu-wrap'><button class='device-menu-button' type='button' aria-label='Device menu' onclick='toggleDeviceMenu(" + idx + ")'>&hellip;</button><div id='device-menu-" + idx + "' class='device-menu' hidden><button type='button' onclick='identifyDevice(" + idx + ")'>" + tr("Identify Device (Orange LED for 10s)", "デバイスを識別（LEDを10秒間オレンジ点灯）") + "</button><a href='/export_device_preset?index=" + idx + "'>" + tr("Export Preset (JSON)", "プリセットをエクスポート（JSON）") + "</a><button type='button' onclick='chooseDevicePreset(" + idx + ")'>" + tr("Import Preset (JSON)", "プリセットをインポート（JSON）") + "</button></div><input id='preset-file-" + idx + "' type='file' accept='application/json,.json' hidden onchange='importDevicePreset(" + idx + ",this)'></div></div>";
  html += "<div id='device-body-" + idx + "' class='device-body'><div class='uid'>" + htmlEscape(setting.identity) + "</div><p id='preset-status-" + idx + "' class='import-status'></p>";
...
void appendTofCard(String& html, const TofSetting& setting, size_t cardIndex,
  ...
  html += "<div class='card device' data-device-index='" + idx + "' data-collapse-key='" + htmlEscape(collapseKey) + "'><div class='device-head'><h2><button id='collapse-" + idx + "' class='collapse-button' type='button' aria-expanded='true' onclick=\"toggleDevice('" + idx + "','" + htmlEscape(collapseKey) + "')\">&#9660;</button>" + chainPositionBadge(portNumber, portIndex, "ToF") + " " + htmlEscape(setting.displayName) + "</h2>";
  html += "<div class='device-menu-wrap'><button class='device-menu-button' type='button' aria-label='Device menu' onclick='toggleDeviceMenu(" + idx + ")'>&hellip;</button><div id='device-menu-" + idx + "' class='device-menu' hidden><button type='button' onclick='identifyDevice(" + idx + ")'>" + tr("Identify Device (Orange LED for 10s)", "デバイスを識別（LEDを10秒間オレンジ点灯）") + "</button><a href='/export_device_preset?index=" + idx + "'>" + tr("Export Preset (JSON)", "プリセットをエクスポート（JSON）") + "</a><button type='button' onclick='chooseDevicePreset(" + idx + ")'>" + tr("Import Preset (JSON)", "プリセットをインポート（JSON）") + "</button></div><input id='preset-file-" + idx + "' type='file' accept='application/json,.json' hidden onchange='importDevicePreset(" + idx + ",this)'></div></div>";
...
void appendJoystickCard(String& html, const JoystickSetting& setting, size_t cardIndex,
  ...
  html+="<div class='card device' data-device-index='"+idx+"' data-collapse-key='"+htmlEscape(collapseKey)+"'><div class='device-head'><h2><button id='collapse-"+idx+"' class='collapse-button' type='button' aria-expanded='true' onclick=\"toggleDevice('"+idx+"','"+htmlEscape(collapseKey)+"')\">&#9660;</button>"+chainPositionBadge(portNumber, portIndex,"Joystick")+" "+htmlEscape(setting.displayName)+"</h2>";
...
#endif
  String html = pageStart("ChainOSCmini Settings");
  html += F("<div id='save-toast' class='toast' hidden></div><h1>ChainOSCmini Settings</h1>");
