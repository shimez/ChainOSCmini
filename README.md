...
Chain DualKey本体のキー／RGB LED、片側Chainポートの列挙に加え、接続したChain Keyの入力とLEDを実機確認するための診断版です。Wi-Fi、OSC、Web UI、設定保存、反対側のChainポート制御はまだ実装していません。
...
実機確認後、`0.5.0`でGPIO47（RX）／GPIO48（TX）側を追加し、左右Chainポートの同時利用と識別方法を検証します。
